...
class ServerRegistry:
    ...
    def getUser(self, uid):
        ...
        if isinstance(uid, str):
            for user in self.users:
                if self.users[user]["description"]['uuid'] == int(uid):
                    return user
        return None
    ...
    def addUser(self, description):
        ...
        if 'signature' in description.keys():
            del description['signature']
        if 'certificate' in description.keys():
            del description['certificate']
        if 'pubKey' in description.keys():
            del description['pubKey']
        if 'chain' in description.keys():
            del description['chain']
    ...
    def sendMessage(self, src, dst, msg, symkey, ctr, signature, cert, chain, receipt):
        nr = "0"
        src = str(src)
        dst = str(dst)
        ...
        try:
            ...
            self.saveOnFile(path + nr + "_scs", symkey + "\n" + ctr + "\n" + signature + "\n" + cert + "\n" + ''.join(chain))
    ...
    def recvMessage(self, uid, msg):
        ...
        try:
            scs = self.readMsgFile(uid, msg+"_scs")
        except:
            logging.exception("Cannot load decryption values")
    ...
    def storeReceipt(self, uid, msg, receipt, certificate, chain):
        print("\n")
        print(chain)
        print("\n")
    ...
    def getReceipts(self, uid, msg):
        ...
        for fname in os.listdir(boxdir):
            ...
            if m and m.group(1) == msg:
                ...
                # Get the correct receipt's sender's id
                tmp = []
                for i in m.group(1):
                    if i != '_':
                        tmp += [i]
                        ...
                        break
                tmp = ''.join(e for e in tmp)
                receipt = {
                    "date": m.group(3), "id": tmp, "receipt": receiptText}
